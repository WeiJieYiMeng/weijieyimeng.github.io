...
var styleFit={
  ...
  paddingMode : (h=1)=>{
    let customPaddingMode = Number(JSON.parse(localStorage.getItem("paddingMode")));
    switch(customPaddingMode){
      case 0:document.documentElement.style.setProperty('--content-max-width', "auto");
      case 3:document.documentElement.style.setProperty("--content-padding","0.618rem 0");
      case 2:document.documentElement.style.setProperty("--reader-padding","0");break;
      case 1:if(h){
        document.documentElement.style.setProperty("--reader-padding","0rem 2rem");
        document.documentElement.style.setProperty("--content-padding","0.618rem 1.618rem");
        document.documentElement.style.setProperty("--content-max-width","800px");
      };break;
    }
    if(h){window.top.hint("边框模式："+window.top.uiPaddingMode(customPaddingMode))}
  },
  chapterAlways : (h=1)=>{
